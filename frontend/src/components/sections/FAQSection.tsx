...
export default function FAQSection() {
  ...
  const popular = faqs.filter(f => f.popular).slice(0, 6)

  return (
    <div className="section-padding bg-gray-50/50 dark:bg-gray-900/30">
      <div className="container-gov max-w-3xl">
        ...
        <div className="space-y-3">
          {popular.map((faq, i) => {
            ...
            return (
              <motion.div key={faq.id} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08, duration: 0.4 }} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 overflow-hidden">
                <button className="w-full flex items-center justify-between gap-4 p-5 text-left hover:bg-gray-50 dark:hover:bg-gray-750 transition-colors" onClick={() => setOpenId(isOpen ? null : faq.id)} aria-expanded={isOpen}>
