...
import { Image, Plus, Edit3, Trash2, X, Loader2, AlertCircle, CheckCircle } from 'lucide-react'
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage gallery images and items</p>
        </div>
          ...
          {items.map((item, idx) => (
            <motion.div key={item._id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: idx * 0.03 }}
              ...
              <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => { setEditing(item); setForm({ title: { ...item.title }, description: { ...item.description }, imageUrl: item.imageUrl, category: item.category, order: item.order }); setShowForm(true) }}
